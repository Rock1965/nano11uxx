...
void setPinModeCallback(byte pin, int mode) {
	...
	case ANALOG:
		if (IS_PIN_ANALOG(pin)) {
			if (IS_PIN_DIGITAL(pin)) {
				nanoBus[pin].input(NEITHER); // set pin as input & disable internal pull-ups
			}
			pinConfig[pin] = ANALOG;
			ad[PIN_TO_ANALOG(pin)].enable();
		}
		break;
	case INPUT:
		if (IS_PIN_DIGITAL(pin)) {
...
void digitalWriteCallback(byte port, int value) {
	uint32_t last = nanoBus.read();
	switch (port) {
	case 0:
		nanoBus = (last & 0xFFFF00) | value;
		break;
	case 1:
		nanoBus = (last & 0xFF00FF) | (value<<8);
		break;
	case 2:
		nanoBus = (last & 0x00FFFF) | (value<<16);
		break;
	default:
		break;
	}
