...
int main(void) {
	...
	//
dbg.waitToDebugMode();  // <-- please enter to debug mode
	uint8_t buf[4];

	// read from EEPROM and check it when power on
	EEPROM::read(0, buf, sizeof(buf));
	...
	buf[2] = 0x55;
	buf[3] = 0x55;
	EEPROM::write(0, buf, sizeof(buf));   // <-- please power off and on (reset) again to check EEPROM contents
